...
//    DLog(@"initWithViewController _defaultNavBarTop ＝%f %@",_defaultNavBarTop,NSStringFromCGRect(scrollView.bounds));
    ...
    if (self) {
        ...
        _DonotshowtopNavNavigationBar = NO;
...
//            DLog(@"deltaY = %f self.navigationBar.frame.origin.y = %f",deltaY,self.navigationBar.frame.origin.y);
//            if (deltaY == 0 && _shouldHideNavigationBarOnScroll) {
...
//        DLog(@"rect fram %@",NSStringFromCGRect(rect));
        if (isNavigationBarExisting && _shouldHideNavigationBarOnScroll){
//            if (navBar.frame.origin.y == 20) {
//                rect.origin.y = 64;
//            }
//            else if(navBar.frame.origin.y == -44)
//            {
//                rect.origin.y = 24;
//            }
            rect.origin.y= MIN(MAX(navBar.top-deltaY -20, _defaultNavBarTop-navBar.height-self.additionalNavBarShiftForIOS7StatusBar-20), _defaultNavBarTop );
            rect.size.height = 572;
            
        }
        else
        {
            if (!_DonotshowtopNavNavigationBar) {
                rect.origin.y = 64;
            }
