...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        [button setFrame:CGRectMake(xOffset,0,
                                    131, kHeightOfTopScrollView)];
  //      DLog(@"xOffset = %.2f",xOffset);
        //计算下一个tab的x偏移量
        xOffset += 131 + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 42, 131, _shadowImage.size.height);
            button.selected = YES;
        }
